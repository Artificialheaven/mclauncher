...
class Launcher:
    ...
    def start_game(self) -> None:
        ...
        # print("\n".join(result))
        # print(result)
        subprocess.run(result, encoding='utf-8')
    ...
    def GenerateCommand(self) -> list[Any]:
        """
        ...
        # 启动器版本（暂未设置自定义）
        version = self.game_version.split('.')

        if int(version[0]) >= 1 and int(version[1]) >= 12:
            self.options[f"-Dminecraft.launcher.brand={self.launcher_name}"] = ""
            self.options[f"-Dminecraft.launcher.version={self.launcher_version}"] = ""
        else:
            # 此处补充natives
        ...
        # 载入固定值（暂未制作自定义）
        self.options[f"-Xmn{self.min_size}"] = ""
        self.options[f"-Xmx{self.max_size}"] = ""
        self.options["-Dlog4j2.formatMsgNoLookups=true"] = ""
        ...
        # 载入游戏名称，可自定义
        print('in', self.options.get('username'))
        ...
        self.options['--uuid'] = self.UUID
        self.options['--accessToken'] = self.ACCESSTOKEN
        self.options['--userType'] = 'mojang'
